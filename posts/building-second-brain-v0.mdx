...
# Building a Semantic Memory Assistant: Second Brain v0
...
## The Problem: Moving Chaos

In summer 2024, I moved across the country. If you've ever done a long-distance move, you know the chaos:
...
- **Spreadsheets**: Too much friction to maintain
- **Notes apps**: Hard to search when you don't remember exact wording
- **Memory**: Not reliable after packing the 47th box

I needed something that understood *what* I was looking for, not just keyword matching. Enter semantic search.

---

## The Approach: RAG Without the G
...
The secret sauce is combining two signals:
...
**Why this works:** When you move the passport from the blue suitcase to your backpack, you don't need to "update" anything—just add a new memory. The recency boost ensures the latest location wins.

### Design Principles for v0

1. **Solve the immediate problem**: Location tracking, nothing more
...
4. **Local-first**: Works on planes, in storage units, anywhere
...
1. **RAG doesn't need generation**: For many use cases, retrieval alone is sufficient
...
4. **Hybrid scoring > pure similarity**: Combining signals beats any single metric

**Product Lessons:**

Building for yourself is liberating. No stakeholders, no feature requests, no scope creep. You can focus purely on solving the immediate problem. The hardest part? Knowing when to stop adding features.
